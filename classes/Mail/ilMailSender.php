...
require_once(__DIR__ . "/../plugin/PluginConfig.php");
...
class ilMailSender implements Mails\interfaces\MailSender {
	...
	public function sendMail($a_template) {
		...
		$message = $a_template->getMailPieces("BODY");
		$subject = html_entity_decode($a_template->getMailPieces("SUBJECT"));
		$csm_conf = new \PluginConfig();
		...
		$iCal = new ICalGenerator($this->crs_id, $this->usr_id, $a_template);
		$attach_file = $a_template->sendAttachment($iCal);
		...
		$sender->Subject = $sender->html2text($subject);
